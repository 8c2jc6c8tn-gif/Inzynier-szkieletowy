...
CHANGELOG:
v2.3.1 (poprawki po testach)
  - FIX (krytyczny, crash): "Dopasuj okapy" zapisywał do okap_lewo/okap_prawo wynik
    obliczeń trygonometrycznych jako liczbę zmiennoprzecinkową, a te pola są typu int
    -- przy powrocie na zakładkę Dach powodowało to StreamlitMixedNumericTypesError
    i wywalenie całej aplikacji. Wynik jest teraz zaokrąglany do int przed zapisem.
  - FIX: suwak i pole liczbowe w Geometrii (Wysokość/Szerokość/Długość) mogły przy
    pierwszym uruchomieniu pokazywać RÓŻNE wartości (suwak np. na minimum, pole na
    właściwej wartości domyślnej) -- brakowało jawnego value= przy tworzeniu widżetów,
    mimo ustawienia stanu sesji. Przywrócone jawne value= (jak w oryginalnym kodzie).
  - FIX: rysunek fundamentu z wymiarami nadal ucinał się po prawej stronie mimo
    wcześniejszej próby centrowania -- zastąpione prawdziwie responsywnym SVG
    (viewBox + width="100%" zamiast sztywnych pikseli), które zawsze mieści się
    w oknie i jest wyśrodkowane, niezależnie od szerokości ekranu
  - Zmieniona nazwa eksportowanego pliku na projekt-fundamentu.pdf
  - Wyśrodkowany napis na przycisku eksportu
v2.3 (model papy + poprawki UI + błąd obliczeniowy)
  - ZMIANA MODELU: papa liczona jest teraz jako ułożona WZDŁUŻ KALENICY (wzdłuż
    dłuższego boku budynku) zamiast w poprzek -- daje to mniej poziomych łączeń,
    zgodnie z praktyką dekarską. Kursy biegną od okapu do kalenicy (ich liczbę
    determinuje długość połaci po skosie), a każdy kurs w razie potrzeby zszywa
    się z kilku rolek na długości (jeśli rolka jest krótsza niż połać).
  - Dodano 5 cm zapasu z każdej strony budynku (okap + oba szczyty) na zawinięcie
    papy w dół -- zabezpieczenie przed zaciąganiem wody w naroża i łączenia,
    widoczne w appce jako informacyjna "chmurka" (st.info) w sekcji Papa.
  - FIX (ważny): "Dopasuj okapy" optymalizował długość połaci TYLKO pod papę wierzchnią,
    ignorując podkładową. Przy różnych parametrach obu warstw mogło to PODNOSIĆ liczbę
    rolek podkładówki (potwierdzone symulacją: 7→8 rolek). Teraz algorytm bierze
    mniejszą z dwóch wartości optymalnych (pod + wierzch), co matematycznie gwarantuje,
    że liczba rolek nigdy nie wzrośnie po kliknięciu "Dopasuj" -- dla żadnej z warstw.
    Po zmianie modelu na "wzdłuż kalenicy" przeliczony pod okapy LEWO/PRAWO (bo to
    one wpływają na długość połaci po skosie, czyli na liczbę kursów/łączeń),
    z poprawnym przelicznikiem trygonometrycznym cm okapu -> metry połaci.
  - Widoczny podział: liczba kursów × rolek na kurs dla obu warstw papy, plus %
    odpadu na kursach osobno dla podkładowej i wierzchniej (przejrzystość: można
    na bieżąco zweryfikować, czy liczby się zgadzają)
  - FIX: rysunek fundamentu z wymiarami był wyrównany do lewej zamiast wyśrodkowany --
    teraz wyśrodkowany i przewijalny w poziomie na wąskich ekranach (responsywność)
  - FIX: eksport raportu fundamentu zmieniony z linku "kliknij żeby wygenerować, potem
    kliknij żeby pobrać" na prawdziwy, duży przycisk pobierania (jedno kliknięcie,
    natywny mechanizm przeglądarki, stała nazwa pliku raport_fundamenty.pdf)
  - FIX: kolorowe karty porównania wariantów (mniej/standard/więcej słupków) były
    wizualnie sklejone -- dodany odstęp między kolumnami i marginesy kart
  - Uwaga: model papy to wciąż uproszczona wersja referencyjna -- w kolejnym kroku
    będzie dopracowany pod konkretne, wybieralne produkty papy
v2.2 (UI/UX)
...
/* Duży, wyróżniony przycisk eksportu/pobierania (np. raport fundamentu) */
div[data-testid="stDownloadButton"] button {
    font-size: 1.15rem;
    font-weight: 700;
    padding: 16px 20px;
    background-color: var(--akcent-niebieski);
    border: none;
    color: white;
    display: flex;
...
def slider_z_wpisem(etykieta, minv, maxv, klucz, step=1, etykieta_liczby="Dokładna wartość"):
    ...
    st.slider(etykieta, minv, maxv, value=st.session_state[klucz_slider], step=step, key=klucz_slider, on_change=_ze_slidera)
    st.number_input(etykieta_liczby, minv, maxv, value=st.session_state[klucz], step=step, key=klucz, on_change=_z_wpisu)
...
ZAPAS_ZAWINIECIE_PAPY_M = 0.05  # 5 cm zapasu z każdej strony na zawinięcie papy w dół (zabezpieczenie przed zaciąganiem wody w łączeniach)

def oblicz_papa():
    """
    Wspólna logika zapotrzebowania na papę (podkładową i wierzchnią) -- używana
    w zakładce Dach i w Kosztorysie, żeby te dwa miejsca nigdy się nie rozjechały.

    Model: papa układana WZDŁUŻ KALENICY (wzdłuż dłuższego boku budynku) -- kursy
    biegną równolegle do kalenicy i pokrywają dach od okapu w górę, każdy kolejny
    kurs zachodzi na poprzedni na szerokość "zakładu". To daje mniej poziomych
    łączeń niż układanie w poprzek. Każdy kurs pokrywa całą długość połaci (wzdłuż
    kalenicy) -- jeśli rolka jest krótsza, kursy zszywa się z kilku rolek na długość.

    Doliczamy 5 cm zapasu z każdej strony budynku (okap i szczyty) na zawinięcie
    papy w dół -- zabezpiecza to przed zaciąganiem wody w naroża i łączenia.
    """
    szer_polaci = (st.session_state.dlug + st.session_state.okap_przod + st.session_state.okap_tyl) / 100
    szer_polaci += 2 * ZAPAS_ZAWINIECIE_PAPY_M  # zapas przy obu szczytach (zawinięcie na krawędziach bocznych)
    dl_pol = dlugosc_polaci() + ZAPAS_ZAWINIECIE_PAPY_M  # zapas przy okapie (zawinięcie na krawędzi dolnej)

    def dla_warstwy(szerokosc_rolki, zaklad, dlugosc_rolki):
        szer_ef = szerokosc_rolki - zaklad          # efektywne pokrycie jednego kursu (po odjęciu zakładu)
        liczba_kursow = math.ceil(dl_pol / szer_ef)  # kursy od okapu do kalenicy
        rolek_na_kurs = math.ceil(szer_polaci / dlugosc_rolki)  # rolki zszywane na długości jednego kursu
        rolki = liczba_kursow * rolek_na_kurs * 2    # dwie połacie dachu
        return szer_ef, liczba_kursow, rolek_na_kurs, rolki

    szer_ef_pod, kursy_pod, rolek_na_kurs_pod, rolki_pod = dla_warstwy(
        st.session_state.szerokosc_rolki_papa_podklad,
        st.session_state.zaklad_papa_podklad,
        st.session_state.dlugosc_rolki_papa_podklad,
    )
    szer_ef_w, kursy_w, rolek_na_kurs_w, rolki_w = dla_warstwy(
        st.session_state.szerokosc_rolki_papa_wierzch,
        st.session_state.zaklad_papa_wierzch,
        st.session_state.dlugosc_rolki_papa_wierzch,
    )

    return {
        'szer_polaci': szer_polaci, 'dl_pol': dl_pol,
        'szer_efekt_pod': szer_ef_pod, 'pasy_pod': kursy_pod, 'rolek_na_kurs_pod': rolek_na_kurs_pod, 'rolki_pod': rolki_pod,
        'szer_efekt_w': szer_ef_w, 'pasy_w': kursy_w, 'rolek_na_kurs_w': rolek_na_kurs_w, 'rolki_w': rolki_w,
    }
...
def dach_tab():
    ...
    if dach_opcje == "Konstrukcja dachu":
        ...
        if st.session_state.pokrycie == "Papa":
            ...
            papa = oblicz_papa()
            szer_efekt_pod = papa['szer_efekt_pod']
            szer_efekt_w = papa['szer_efekt_w']
            dl_pol_z_zapasem = papa['dl_pol']
            kursy_pod = papa['pasy_pod']
            kursy_w = papa['pasy_w']

            # Papa biegnie wzdłuż kalenicy -- liczbę kursów (a więc i poziomych łączeń)
            # określa długość połaci PO SKOSIE (dl_pol), na którą wpływają okapy LEWO/PRAWO
            # (a nie przód/tył, jak było liczone poprzednio -- to był błąd). Docelową długość
            # bierzemy jako MNIEJSZĄ z dwóch wartości optymalnych (podkładowa/wierzchnia),
            # więc liczba rolek nigdy nie wzrośnie po kliknięciu "Dopasuj".
            opt_dl_pol_pod = kursy_pod * szer_efekt_pod
            opt_dl_pol_w = kursy_w * szer_efekt_w
            cel_dl_pol = min(opt_dl_pol_pod, opt_dl_pol_w)
            roznica_m = cel_dl_pol - dl_pol_z_zapasem

            odpad_pod_proc = (opt_dl_pol_pod - dl_pol_z_zapasem) / dl_pol_z_zapasem * 100 if dl_pol_z_zapasem > 0 else 0
            odpad_w_proc = (opt_dl_pol_w - dl_pol_z_zapasem) / dl_pol_z_zapasem * 100 if dl_pol_z_zapasem > 0 else 0

            with sekcja("📦 Zapotrzebowanie na papę"):
                st.info(
                    "ℹ️ Papa układana jest **wzdłuż kalenicy** (wzdłuż dłuższego boku) -- mniej "
                    "poziomych łączeń niż przy układaniu w poprzek. Do wymiarów doliczamy **5 cm "
                    "zapasu z każdej strony budynku** na zawinięcie papy w dół, co zabezpiecza "
                    "przed zaciąganiem wody w naroża i łączenia."
                )
                st.caption(f"Odpad na kursach (łączenia poziome): podkładowa {odpad_pod_proc:.1f}% • wierzchnia {odpad_w_proc:.1f}%")

                if roznica_m < 0.001:
                    st.success("✅ Okapy (lewo/prawo) są optymalne dla obu warstw papy -- minimalna liczba łączeń.")
                else:
                    st.error(f"⚠️ Można zmniejszyć liczbę łączeń, wydłużając okap (lewo/prawo) łącznie o: {roznica_m*100:.1f} cm")
                    if st.button("🎯 Dopasuj okapy", use_container_width=True):
                        st.session_state.pokaz_wybor = True
                    if st.session_state.pokaz_wybor:
                        st.caption("Papa biegnie wzdłuż kalenicy, więc na liczbę łączeń wpływają tylko okapy Lewo/Prawo.")
                        lewo = st.checkbox("Lewo", True, key='opt_lewo')
                        prawo = st.checkbox("Prawo", True, key='opt_prawo')
                        rowno = st.checkbox("Równomiernie", True, key='opt_rownomiernie')
                        if st.button("✅ Zastosuj", key='zastosuj_opt'):
                            # przeliczenie różnicy w dl_pol (metry, po skosie) na potrzebny
                            # przyrost sumy okap_lewo + okap_prawo (cm, w rzucie poziomym)
                            przyrost_cm = roznica_m * 200 * math.cos(math.radians(st.session_state.kat))
                            liczba = sum([lewo, prawo])
                            if liczba:
                                delta = przyrost_cm / (liczba if rowno else 1)
                                if lewo: st.session_state.okap_lewo = int(round(max(0, min(100, st.session_state.okap_lewo + delta))))
                                if prawo: st.session_state.okap_prawo = int(round(max(0, min(100, st.session_state.okap_prawo + delta))))
                                st.session_state.pokaz_wybor = False
                                st.rerun()
                # jeśli zmienialiśmy okapy powyżej, liczymy jeszcze raz na świeżych wartościach
                papa = oblicz_papa()
                st.write(f"**Papa podkładowa:** {papa['rolki_pod']} rolek ({papa['pasy_pod']} kursów × {papa['rolek_na_kurs_pod']} rolki/kurs)")
                st.write(f"**Papa wierzchnia:** {papa['rolki_w']} rolek ({papa['pasy_w']} kursów × {papa['rolek_na_kurs_w']} rolki/kurs)")
...
def fundamenty_tab():
    ...
    with col_o1:
        ...
        if zalecany == "mniej":
            st.markdown(f"""
            <div style="background-color:{kolor}; padding:14px; border-radius:12px; text-align:center; color:white; margin:6px 2px;">
            ...
            st.markdown(f"""
            <div style="background-color:{kolor}; padding:14px; border-radius:12px; text-align:center; color:white; opacity:0.8; margin:6px 2px;">
            <h3 style="margin:0; color:white;">🔴 Mniej słupków</h3>
            <p style="font-size:16px; margin:8px 0;"><b>{ile_mniej} szt.</b> | obc. {obc_mniej:.1f} kN | zapas {zapas_mniej:.0f}%</p>
            <p style="font-size:16px; margin:4px 0;">Smukłość {sm_mniej:.0f} | Wyboczenie {wyb_mniej:.0f}%</p>
            </div>
    ...
    with sekcja("🗺️ Rysunek fundamentu z wymiarami"):
        ...
        promien = 10
        calkowita_szer = szer_px + 80
        calkowita_wys = dlug_px + 80

        kolory_wariantow = {"mniej": "#e74c3c", "standard": "#f39c12", "wiecej": "#2980b9"}
        kolor_slupkow = kolory_wariantow[wybor]

        # viewBox + width="100%" zamiast sztywnych pikseli -- rysunek SKALUJE SIĘ
        # do szerokości kontenera (nigdy nie jest ucinany), a max-width pilnuje,
        # żeby na szerokich ekranach nie zrobił się nienaturalnie ogromny.
        svg = (
            f'<svg viewBox="0 0 {calkowita_szer} {calkowita_wys}" width="100%" '
            f'style="max-width:{calkowita_szer}px; height:auto; display:block; margin:0 auto;" '
    ...
    with sekcja("📄 Eksport"):
        ...
        if pdf_bytes:
            # Prawdziwy przycisk pobierania -- jedno kliknięcie od razu ściąga plik,
            # bez pośredniego linku i bez stałej, przewidywalnej nazwy pliku.
            st.download_button(
                label="📥  Eksportuj projekt fundamentu",
                data=pdf_bytes,
                file_name="projekt-fundamentu.pdf",
                mime="application/pdf",
                use_container_width=True,
                key="pobierz_raport_fundamenty",
            )
